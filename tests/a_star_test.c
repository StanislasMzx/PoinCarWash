...
describe(test_a_star)
{
    it("Conformance test")
    {
        ...
        *one_vehicle = vehicle_find_by_name("Tesla Model 3");
        ...
        char *start_key = "FROTHPOTHR18051FR*SOD*S*OTHR*180*5*_*_";

        List_t *one_list = a_star_list(table, start_key, end_key, one_vehicle);

        asserteq_str("FROTHPOTHR18111FR*SOD*S*OTHR*181*1*_*_", one_list->list[0].key);
        asserteq_str("FROTHPOTHR18051FR*SOD*S*OTHR*180*5*_*_", one_list->list[1].key);
