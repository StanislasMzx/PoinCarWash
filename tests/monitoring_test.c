...
describe(test_load_network)
{
    it("Conformance test")
    {
        ...
        asserteq_str(network[0].journey->list[3].value->name, "SIEG63 - ePremium - Montpeyroux - Parking Entree");
        asserteq_str(network[1].journey->list[1].value->name, "Echirolles, 10 Cours Jean Jaurès");
        asserteq_str(network[1].journey->list[2].value->name, "Chaintre, place du luminaire");
        asserteq_str(network[1].journey->list[3].value->name, "RELAIS DE LA COULINE");
        for (int i = 0; i < 2; i++)
        {
            list_destroy(network[i].journey);
        }
        free(network);
        table_destroy(table);
    }
